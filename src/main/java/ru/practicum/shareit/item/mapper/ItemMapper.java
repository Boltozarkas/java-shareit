...
public class ItemMapper {
    ...
    public BookingInfo toBookingInfo(Booking booking) {
        ...
        Long bookerId = Optional.ofNullable(booking.getBooker())
                .map(User::getId)
                .orElseThrow(() -> new IllegalArgumentException("Booking has no booker"));
    ...
    /**
     * Группирует комментарии по ID предметов
     */
    ...
    /**
     * Находит последнее бронирование для каждого предмета
     */
    ...
    /**
     * Находит следующее бронирование для каждого предмета
     */
